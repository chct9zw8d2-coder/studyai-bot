...
def get_image_cache(key: str, *, ttl_days: int | None = None):
    init_db()
    with _conn() as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            if ttl_days:
                cur.execute(
                    "SELECT telegram_file_id, created_at FROM image_cache WHERE key=%s AND created_at >= (CURRENT_TIMESTAMP - INTERVAL '%s days')",
                    (key, int(ttl_days)),
                )
            else:
                cur.execute("SELECT telegram_file_id, created_at FROM image_cache WHERE key=%s", (key,))
            row = cur.fetchone()
            if row:
                cur.execute("UPDATE image_cache SET hits = hits + 1, last_hit = CURRENT_TIMESTAMP WHERE key=%s", (key,))
                conn.commit()
            return row

def set_image_cache(key: str, telegram_file_id: str):
    init_db()
    with _conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO image_cache (key, telegram_file_id)
                VALUES (%s,%s)
                ON CONFLICT (key) DO UPDATE SET
